...
def save_to_gofile_history(filename, download_page, size, file_id=None, direct_link=None):
    """Save upload info to local history file with better error handling"""
    try:
        import time
        # Ensure the directory exists
        history_path = GOFILE_HISTORY_FILE
        ...
        # Check for duplicates by file_id or link
        is_duplicate = any(item.get("id") == entry["id"] or item.get("link") == entry["link"] for item in history)
        
        if not is_duplicate:
            # Insert at the very beginning (newest first)
            history.insert(0, entry)
            # Increase limit to 500 so you don't lose files
            history = history[:500] 
            
            with open(history_path, 'w') as f:
                json.dump(history, f, indent=4)
            print(f"✅ History Updated: {filename}")
...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth or not (auth.username == "admin" and auth.password == APP_PASSWORD):
            # This triggers the browser's native login popup
            return Response(
                'Please login with "admin" and your password.', 401,
                {'WWW-Authenticate': 'Basic realm="Login Required"'}
            )
        return f(*args, **kwargs)
    return decorated

# Note: /login and /logout routes are no longer needed for browser-native auth
# -----------------------------
...
GOFILE_ACCOUNT_ID = os.environ.get("GOFILE_ACCOUNT_ID") or ""
GOFILE_API_TOKEN = os.environ.get("GOFILE_API_TOKEN") or ""
GOFILE_FOLDER_ID = os.environ.get("GOFILE_FOLDER_ID") or ""
GOFILE_FOLDER_ID_ALT = os.environ.get("GOFILE_FOLDER_ID_ALT") or ""
            ...
            <div id="encoding-options" style="display: {% if codec != 'none' %}block{% else %}none{% endif %};">
                <div id="video-encoding-options">
                    ...
                    <select name="scale">
                        ...
                        <!-- Horizontal -->
                        <option value="1920:-2">1080p (Landscape)</option>
                        <option value="1280:-2">720p (Landscape)</option>
                        <option value="854:-2">480p (Landscape)</option>
                        <option value="640:-2">360p (Landscape)</option>

                        <!-- Vertical -->
                        <option value="-2:1280">720p Vertical (720x1280)</option>
                        <option value="-2:854">480p Vertical (480x854)</option>
                    </select><br>
...
def upload_to_gofile(file_path, filename, q):
    """Upload file to Gofile.io specific folder"""
    try:
        ...
        if server_data.get("status") != "ok" or not server_data.get("data", {}).get("servers"):
            q.put({"error": "Failed to get upload server"})
            q.put({"log": "DONE"})
            return
        ...
        with open(file_path, 'rb') as f:
            ...
            response = requests.post(
                upload_url,
                files=files,
                data=data, # Send the folderId in the data field
                headers=headers,
                timeout=600 
            )
        ...
        if upload_result.get("status") == "ok":
            ...
            real_server = upload_result["data"].get("servers", [upload_server])[0]
            ...
            q.put({
                "log": f"Success! File saved to Gofile Folder: {filename}",
                "final_url": download_page
            })
        else:
            q.put({"error": f"Upload failed: {upload_result.get('status')}"})
...
def upload_to_gofile_alt(file_path, filename, q):
    """Upload file to the SECOND Gofile folder with direct link support"""
    try:
        if not GOFILE_API_TOKEN:
            q.put({"error": "Gofile API token not configured"})
            q.put({"log": "DONE"})
        ...
        q.put({"stage": "Getting Gofile server...", "percent": 10})
        server_resp = requests.get("https://api.gofile.io/servers", headers={"Authorization": f"Bearer {GOFILE_API_TOKEN}"}, timeout=10)
        server_resp.raise_for_status()
        upload_server = server_resp.json()["data"]["servers"][0]["name"]
        
        upload_url = f"https://{upload_server}.gofile.io/uploadFile"

        with open(file_path, 'rb') as f:
            data = {'folderId': GOFILE_FOLDER_ID_ALT} 
            files = {'file': (filename, f)}
            headers = {"Authorization": f"Bearer {GOFILE_API_TOKEN}", "X-Website-Token": get_gofile_website_token()}

            q.put({"stage": "Uploading to Gofile 2 folder...", "percent": 40})
            response = requests.post(upload_url, files=files, data=data, headers=headers, timeout=600)
        ...
        if res.get("status") == "ok":
            ...
            file_id_rec = res["data"].get("id") or res["data"].get("code")
            file_size = os.path.getsize(file_path)
            
            # --- START OF FIX: CONSTRUCT DIRECT LINK ---
            direct_link = None
            # Get storage server (sometimes different from upload server)
            real_server = res["data"].get("servers", [upload_server])[0]
            
            if real_server and file_id_rec:
                safe_url_name = quote(filename)
                direct_link = f"https://{real_server}.gofile.io/download/web/{file_id_rec}/{safe_url_name}"
                q.put({"log": f"Gofile 2 Direct link: {direct_link}"})
            # --- END OF FIX ---

            # Now pass 'direct_link' as the 5th parameter
            save_to_gofile_history(filename, download_page, file_size, file_id_rec, direct_link)
            
            q.put({"stage": "✅ Gofile 2 Upload Complete!", "percent": 100})
            q.put({"log": f"Success! Saved to second folder: {download_page}", "final_url": download_page})
        else:
            q.put({"error": f"Upload failed: {res.get('status')}"})
    except Exception as e:
        q.put({"error": f"Gofile 2 upload failed: {str(e)}"})
    finally:
        q.put({"log": "DONE"})
...
def download_file_directly(url,
    ...
    try:
        ...
        if username or password:
            auth_tuple = (username if username else "", password if password else "")
            q.put({"log": f"Using provided credentials: {username or 'anonymous'}"})
        ...
        is_gofile = "gofile.io" in url.lower()

        # --- Original Gofile PHASE 1: keep as-is (will log if fails) ---
        if is_gofile:
            try:
                q.put({"log": "Performing Gofile handshake..."})
                wt = get_gofile_website_token()  # if this fails → exception caught below
                sess.headers.update({'X-Website-Token': wt})

                # Try guest account (many links work without it now)
                token_resp = sess.post("https://api.gofile.io/accounts", timeout=10)
                if token_resp.status_code == 200:
                    guest_token = token_resp.json().get("data", {}).get("token")
                    if guest_token:
                        sess.headers.update({"Authorization": f"Bearer {guest_token}"})
                        sess.cookies.set("accountToken", guest_token, domain="gofile.io")
                        q.put({"log": "✅ Account token acquired"})
                else:
                    q.put({"log": f"Guest account step skipped (HTTP {token_resp.status_code})"})

                # Optional content pre-authorize
                content_id_match = re.search(r'/web/([^/]+)', url)
                if content_id_match:
                    content_id = content_id_match.group(1)
                    q.put({"log": f"Authorizing Content ID: {content_id[:8]}..."})
                    sess.get(f"https://api.gofile.io/contents/{content_id}", timeout=10)

            except Exception as te:
                q.put({"log": f"Note: Gofile setup warning: {te} — continuing anyway"})

        # ────────────────────────────────────────────────
        # CONNECT + 401 fallback credentials
        # ────────────────────────────────────────────────
        current_url = url
        response = None
        ...
        credentials_list = [auth_tuple] if auth_tuple else [None]
        fallback_idx = 0

        for jump in range(5):  # slightly more attempts allowed
            q.put({"log": f"Connecting to storage server ({jump+1}/5)..."})

            current_auth = credentials_list[-1]
            ...
            if response.status_code in [301, 302, 303, 307, 308]:
                ...
                if is_gofile and "gofile.io/d/" in location:
                    q.put({"log": "⚠️ Link locked (/d/). Refreshing session..."})
                    try:
                        sess.get(location, timeout=10)
                    except:
                        pass
                    current_url = url
                ...
                # Safer relative → absolute
                current_url = urljoin(current_url, location)
                time.sleep(0.8)
                continue

            # Handle 401 → try fallback
            if response.status_code == 401:
                q.put({"log": "401 Unauthorized → trying credentials"})

                if fallback_idx >= len(fallback_credentials):
                    raise ValueError("All fallback credentials failed. Provide correct username/password.")

                next_cred = fallback_credentials[fallback_idx]
                credentials_list.append(next_cred)
                fallback_idx += 1

                u, p = next_cred
                q.put({"log": f"Trying fallback: {u} / {'*' * len(p)}"})
                time.sleep(1.5)  # avoid hammering server
                continue

            # Success (200) or other final status → break
            break
        ...
        if not response or response.status_code != 200:
            ...
            msg = f"Status {status}. "
            if is_gofile:
                msg += "The link may have expired (~1h for direct) or be blocked."
            if response and response.status_code == 401:
                msg += "Authentication failed after all attempts."
            raise ValueError(msg)

        # Original HTML block check
        ctype = response.headers.get('content-type', '')
        if 'text/html' in ctype:
            raise ValueError("Received HTML (error page). Gofile blocked direct link. Get fresh 'Direct' link.")

        # --- PHASE 3: FILENAME (original) ---
        filename = "download"
        cd_header = response.headers.get('content-disposition')
        if cd_header:
            m = re.search(r"filename\*=(?:UTF-8'')?([^;]+)", cd_header, re.IGNORECASE) or \
                re.search(r'filename="([^"]+)"', cd_header)
            if m:
                filename = unquote(m.group(1))

        if filename == "download":
            filename = unquote(urlparse(current_url).path.split('/')[-1])

        filename = os.path.basename(filename)
        if not filename:
            filename = "file.bin"

        q.put({"log": f"Starting download: {filename}"})

        # --- PHASE 4: DISK WRITE (original) ---
        safe_name = get_safe_filename(filename)
        final_path = get_unique_filepath(os.path.join(DOWNLOAD_FOLDER, safe_name))
...
<div class="container">
	...
	<button onclick="batchUploadGofileAlt()" style="background-color: #8e44ad; margin-left:5px;">Upload to Go2-Adult</button>
    ...
    <tbody>
        ...
        <tr draggable="true" class="file-row" data-filepath="{{ item.path }}" data-is-media="{{ item.is_media|lower }}">
		    ...
		    <form method="POST" action="{{ url_for('upload_to_gofile_file_alt') }}" style="display:inline;"><input type="hidden" name="filepath" value="{{ item.path }}"><button type="submit" style="background-color: #8e44ad;">📤 Go2-Adult</button></form>
                    {% if item.is_media %}
...
<script>
    ...
    function batchUploadGofileAlt() {
    	const files = getSelectedFiles();
    	if (files.length === 0) { alert('No files selected'); return; }
    	if (!confirm(`Upload ${files.length} file(s) to Gofile 2?`)) return;

    	const form = document.createElement('form');
    	form.method = 'POST';
    	form.action = '{{ url_for("batch_upload_gofile_alt") }}';
    	files.forEach(file => {
            const input = document.createElement('input');
      	    input.type = 'hidden';
            input.name = 'files[]';
...
@app.route("/upload_to_gofile_alt", methods=["POST"])
@login_required
def upload_to_gofile_file_alt():
    filepath = request.form.get("filepath")
    if not filepath: return redirect(url_for('list_files'))
    full_path = os.path.join(DOWNLOAD_FOLDER, filepath)
    filename = os.path.basename(filepath)
    args = (full_path, filename, progress_queue)
    start_task(upload_to_gofile_alt, args)
    return render_template_string(FILE_OPERATION_TEMPLATE, operation_title=f"Gofile 2 Upload: {filename}", download_started=True, current_path=os.path.dirname(filepath))

@app.route("/batch_upload_gofile_alt", methods=["POST"])
@login_required
def batch_upload_gofile_alt():
    files = request.form.getlist("files[]")
    if not files: return redirect(url_for('list_files'))

    def batch_worker():
        for filepath in files:
            full_path = os.path.join(DOWNLOAD_FOLDER, filepath)
            if os.path.exists(full_path):
                upload_to_gofile_alt(full_path, os.path.basename(filepath), progress_queue)

    start_task(batch_worker, ())
    return render_template_string(FILE_OPERATION_TEMPLATE, operation_title=f"Batch Gofile 2 Upload", download_started=True, current_path="")
